...
import { Plus, Edit, Trash2, Search, Package, Tag, Eye, EyeOff, Star } from 'lucide-react';
...
export default function AdminProducts() {
  ...
  const toggleActive = async (product) => {
    try {
      await axios.put(`/api/products/${product.id}`, { active: product.active ? 0 : 1 });
  ...
  const toggleFeatured = async (product) => {
    try {
      await axios.put(`/api/products/${product.id}`, { featured: product.featured ? 0 : 1 });
  ...
  const toggleBestSeller = async (product) => {
    try {
      await axios.put(`/api/products/${product.id}`, { best_seller: product.best_seller ? 0 : 1 });
      toast.success(`Best Seller ${product.best_seller ? 'removed' : 'added'}`);
      fetchProducts();
    } catch {
      toast.error('Failed to update product');
    }
  ...
  const toggleNewLaunch = async (product) => {
    try {
      await axios.put(`/api/products/${product.id}`, { new_launch: product.new_launch ? 0 : 1 });
      toast.success(`New Launch ${product.new_launch ? 'removed' : 'added'}`);
      fetchProducts();
    } catch {
      toast.error('Failed to update product');
    }
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1 mt-0.5">
                            ...
                            {product.best_seller === 1 && (
                              <span className="text-xs text-orange-600 font-medium">🔥 Best Seller</span>
                            )}
                            {product.new_launch === 1 && (
                              <span className="text-xs text-green-600 font-medium">🆕 New Launch</span>
                            )}
                          </div>
                        </div>
                      </div>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          title={product.best_seller ? 'Remove Best Seller' : 'Mark as Best Seller'}
                          className={`p-1.5 rounded-lg transition-colors text-sm ${product.best_seller ? 'text-orange-500 hover:bg-orange-50' : 'text-gray-300 hover:text-orange-400 hover:bg-orange-50'}`}
                        >
                          🔥
                        </button>
                        <button
                          onClick={() => toggleNewLaunch(product)}
                          title={product.new_launch ? 'Remove New Launch' : 'Mark as New Launch'}
                          className={`p-1.5 rounded-lg transition-colors text-sm ${product.new_launch ? 'text-green-500 hover:bg-green-50' : 'text-gray-300 hover:text-green-400 hover:bg-green-50'}`}
                        >
                          🆕
                        </button>
                        <button
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full shadow-xl">
            ...
            <p className="text-gray-500 text-sm mb-6">This will permanently delete the product and all its images. This action cannot be undone.</p>
